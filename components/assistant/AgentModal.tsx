...
export default function AgentModal({ agent, onClose }: Props) {
 ...
 async function handleGenerate(mediaBase64?: string) {
  ...
  try {
    ...
    setMessages([firstMsg, { role: "assistant", content: result }]);
  ...
  async function handleSend(text: string) {
    ...
    try {
      const result = await callAgentWebhook(agent, formData, newMessages);
            ...
            <div>
              ...
              <div className="flex gap-2 mt-0.5">
                {(["form", "chat"] as const).map((s) => (
                  <button
                    key={s}
                    onClick={() => s === "chat" && messages.length > 0 ? setStep("chat") : s === "form" ? setStep("form") : null}
